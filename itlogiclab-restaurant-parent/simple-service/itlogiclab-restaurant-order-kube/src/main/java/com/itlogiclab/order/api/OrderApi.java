...
import com.itlogiclab.order.modal.KubeEnvModal;
...
private static Logger logger = LogManager.getLogger(OrderApi.class);
	...
	public ResponseEntity<OrderModal> createNewOrder(@RequestBody OrderModal modal){
		try {
			...
			logger.error("Error occoured while creating new order: "+e.getMessage());
	...
	public ResponseEntity<OrderModal> updateOrder(@RequestBody OrderModal order){
		try {
			...
			logger.error("Error occoured while update the order: order id {} : ", order.getOrderId() +e.getMessage());
			return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
		}
	}
	...
	public ResponseEntity<List<OrderModal>> getAllOrder(){
		try {
			...
			logger.error("Error occoured while fetching all orders from DB: "+e.getMessage());
			return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
	...
	public ResponseEntity<OrderModal> getOrder(@PathVariable Long id){
			...
			logger.error("Error occoured while fetching order with id {} from DB: ",id +e.getMessage());
			return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
		}
	}
	
	@GetMapping(value="/getEnv", produces = MediaType.TEXT_PLAIN_VALUE )
	public ResponseEntity<String> getEnvVariables(){
		logger.debug("Invoking getEnvVariables for testing kubernetes: ");
		return ResponseEntity.ok(modal.getEnvVariables());
	}
